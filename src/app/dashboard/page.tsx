...
interface DashboardData {
  ...
  disponiveis: number
  total: number
  workload: { id: string; name: string; avatar: string; count: number; atrasados: number }[]
  recent: any[]
}
...
export default function DashboardPage() {
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh' }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <p style={{ color: '#64748b', fontSize: 14 }}>A carregar dashboard…</p>
      </div>
  ...
  const kpis = [
    { label: 'Em Produção',      value: data.ativos,          color: '#1d4ed8', bg: '#eff6ff',  link: '/projetos?status=EM_PRODUCAO' },
    { label: 'Atrasados',        value: data.atrasados,       color: '#dc2626', bg: '#fef2f2',  link: '/projetos?status=ATRASADO' },
    { label: 'Concluídos Hoje',  value: data.concluidosHoje,  color: '#16a34a', bg: '#f0fdf4',  link: '/projetos?status=CONCLUIDO' },
    { label: 'Sem Confirmação',  value: data.naoConfirmados,  color: '#d97706', bg: '#fffbeb',  link: '/projetos?status=DELEGADO' },
    { label: 'Disponíveis',      value: data.disponiveis,     color: '#475569', bg: '#f8fafc',  link: '/projetos?status=DISPONIVEL' },
    { label: 'Total de Projetos',value: data.total,           color: '#7c3aed', bg: '#f5f3ff',  link: '/projetos' },
  ]
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 20 }}>
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          <div style={{ padding: '16px 20px', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h2 style={{ fontSize: 15, fontWeight: 700, margin: 0, color: '#0f172a' }}>Projetos Recentes</h2>
            <a href="/projetos" style={{ fontSize: 13, color: '#1d4ed8', fontWeight: 600, textDecoration: 'none' }}>Ver todos</a>
          </div>
          <table>
            <thead>
              <tr>
        ...
        {/* Workload */}
        <div className="card">
          <h2 style={{ fontSize: 15, fontWeight: 700, margin: '0 0 16px', color: '#0f172a' }}>Carga de Trabalho</h2>
              ...
              <div key={w.id}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    {w.atrasados > 0 && (
                      <span style={{ fontSize: 11, background: '#fee2e2', color: '#dc2626', padding: '2px 6px', borderRadius: 999, fontWeight: 700 }}>
                        {w.atrasados} atr.
                      </span>
                    )}
                  </div>
          ...
          {(data.atrasados > 0 || data.naoConfirmados > 0) && (
            <div style={{ marginTop: 20, borderTop: '1px solid #f1f5f9', paddingTop: 16 }}>
              <h3 style={{ fontSize: 13, fontWeight: 700, color: '#0f172a', margin: '0 0 8px' }}>Alertas</h3>
              {data.atrasados > 0 && (
                <div style={{ padding: '10px 12px', background: '#fef2f2', borderRadius: 8, marginBottom: 6, fontSize: 13, color: '#dc2626', fontWeight: 500 }}>
                  {data.atrasados} projeto{data.atrasados > 1 ? 's' : ''} atrasado{data.atrasados > 1 ? 's' : ''}
                </div>
              )}
              {data.naoConfirmados > 0 && (
                <div style={{ padding: '10px 12px', background: '#fffbeb', borderRadius: 8, fontSize: 13, color: '#d97706', fontWeight: 500 }}>
